...
class SilktideCookieBanner {
  ...
  /**
   * Programmatically accept a specific cookie type and trigger its callback
   */
  acceptCookieType(cookieId) {
    const type = this.config.cookieTypes.find((t) => t.id === cookieId);
    if (!type) return;
    ...
    // Sync banner & modal if they exist
    this.removeBanner();
    this.hideBackdrop();
    this.showCookieIcon();
    this.updateCheckboxState();
  ...
  getModalContent() {
    ...
    const preferencesDescription =
      ...
      "<p>Wir respektieren Ihr Recht auf Privatsphäre. Sie können entscheiden, welche Arten von Cookies und externen Diensten Sie zulassen möchten. Ihre Auswahl gilt für unsere gesamte Website.</p>";
    ...
    const modalContent = `
      ...
      <footer>
        ${acceptAllButton}
        ${rejectNonEssentialButton}
        <button class="preferences-save st-button st-button--secondary">Auswahl speichern</button>
      </footer>
    `;
  ...
  savePreferences() {
    ...
    // Save current checkbox states to storage and run callbacks
  ...
  setupEventListeners() {
    ...
    if (this.modal) {
      ...
      // Remove the direct localStorage update from the checkboxes
      // We only want to update when the user clicks 'Save', 'Accept All', or 'Reject All'
    }
  ...
  preventBodyScroll() {
    ...
    // Use top offset instead of position:fixed to avoid scroll-to-top
...
(function () {
  ...
  window.silktideCookieBannerManager.acceptCookieType = (id) => {
    if (cookieBanner) cookieBanner.acceptCookieType(id);
  };
