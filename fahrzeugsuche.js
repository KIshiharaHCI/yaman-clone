...
async function loadVehicles() {
  ...
  try {
    ...
    loading.innerHTML =
      '<p style="color: var(--gold);">Fehler beim Laden der Fahrzeuge. Bitte versuchen Sie es später erneut.</p>';
  }
}
...
function populateModelDropdown() {
  ...
  const models = [
    ...new Set(
      allVehicles
        .map((v) => v.modelName || v.modelGroupName || "")
        .filter(Boolean),
    ),
  ].sort();
...
function initCheckboxes() {
  document.querySelectorAll('input[data-filter="kraftstoff"]').forEach((cb) => {
    cb.addEventListener("change", () => {
      activeFilters.kraftstoff = [
        ...document.querySelectorAll('input[data-filter="kraftstoff"]:checked'),
      ].map((c) => c.value);
      applyFiltersAndRender();
    });
  });
  ...
  document.querySelectorAll('input[data-filter="getriebe"]').forEach((cb) => {
    cb.addEventListener("change", () => {
      activeFilters.getriebe = [
        ...document.querySelectorAll('input[data-filter="getriebe"]:checked'),
      ].map((c) => c.value);
      applyFiltersAndRender();
    });
  });
