...
function formatMakeDisplay(make) {
  ...
  return (
    map[make] || make.charAt(0) + make.slice(1).toLowerCase().replace(/_/g, " ")
  );
}
...
async function loadVehicle() {
  ...
  try {
    ...
    loadingEl.innerHTML =
      '<p style="color: var(--gold);">Fehler beim Laden des Fahrzeugs.</p><a href="fahrzeugsuche.html" style="color: var(--gold);">&larr; Zurück</a>';
  }
}
...
function populateSpecs(v) {
  ...
  if (v.registration || v.firstRegistration) {
    specs.push({
      label: "EZ",
      value: formatRegistration(v.registration || v.firstRegistration),
    });
  }
  ...
  if (fuel) {
    ...
    if (v.emissionStandard)
      fuelStr += ` | ${formatEmissionStandard(v.emissionStandard)}`;
  ...
  if (v.mileage || v.mileageKm) {
    ...
    specs.push({
      ...
      value: Number(km).toLocaleString("de-DE") + " km",
    });
  }
  ...
  if (v.enginePower) {
    specs.push({
      label: "Leistung",
      value: `${v.enginePower} kW (${kwToPs(v.enginePower)} PS)`,
    });
  }
  ...
  if (v.categories && v.categories.length > 0) {
    specs.push({
      ...
      value: formatCategory(v.categories),
    });
  }
  ...
  if (v.numberOfCylinders && v.engineSize) {
    specs.push({
      label: "Motor",
      value: `${v.numberOfCylinders} Zylinder | ${Number(v.engineSize).toLocaleString("de-DE")} cm³`,
    });
  }
...
function parseEquipmentFromDescription(html) {
  ...
  tmp.querySelectorAll("li").forEach((li) => {
    ...
    if (
      text &&
      text.length > 2 &&
      !text.includes("Zwischenverkauf") &&
      !text.includes("Fahrzeugbeschreibung dient")
    ) {
      items.push(text);
    }
  });
